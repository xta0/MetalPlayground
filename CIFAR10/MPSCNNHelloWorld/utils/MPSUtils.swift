...
enum MPSUtils {
    ...
    static func offsetForFC(_ inputShape:(Int,Int)) -> MPSOffset {
        return MPSOffset(
            x: inputShape.0/2,
            y: inputShape.1/2,
            z: 0
...
struct MPSImageWrapper {
    ...
    var image: MPSImage?
    var temp_image: MPSTemporaryImage?
    var desc: MPSImageDescriptor
    
    init(device: MTLDevice, n: Int, c: Int, h: Int, w:Int, temporary: Bool = false, commandBuffer: MTLCommandBuffer?=nil) {
        self.device = device
        ...
        if !temporary {
            ...
            self.temp_image = MPSTemporaryImage(commandBuffer: commandBuffer!, imageDescriptor: desc)
        }
    }
}
